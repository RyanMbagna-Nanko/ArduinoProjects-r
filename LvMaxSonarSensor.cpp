...
void LvMaxSonarSensor::setReadPin(int readPinIn, SensorMode sensorModeIn)
{
  ...
  sensorMode = sensorModeIn;
   digitalWrite(COLLISION_AVOID_SENSOR_ENABLE_PIN, HIGH); //BX Pin always set to high HIGH
}

void LvMaxSonarSensor::enableReadPin()
{
 digitalWrite(Left_RX_PIN, HIGH);
  pinMode(Right_TX_PIN, INPUT);
}

void LvMaxSonarSensor::disableReadPin()
{
digitalWrite(Left_RX_PIN, LOW);
}
...
long LvMaxSonarSensor::getDistanceCm()
{
  ...
  else  // ANALOG_MODE
  {
    ...
    long cmReading;
    long pulseduration;
    pulseduration = pulseIn(Right_TX_PIN, HIGH);
    //anVolt = (analogRead(readPin)/2) * 2.54;
    cmReading = analogRead(readPin) * 1.27;  // cm
   
// 9.8 mV / in = 3.85282 mV / cm
// 3.8582mV / cm = 0.0038582V / cm
